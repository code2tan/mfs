"""ReadService: search and content retrieval for indexed and live objects.

Handles search (semantic/keyword/hybrid), path listing, content reading (cat/head/tail),
pattern matching (grep), and full export. Supports both pipeline-indexed content
(Milvus vector search) and live connector reads (with artifact cache for converted content).
"""
...
_HEAD_CACHE_N = 100
_BARE_CAT_MAX_BYTES = 5 * 1024 * 1024
_GREP_LINEAR_SCAN_MAX = 200
...
class ReadService:
    """Search and content retrieval for registered connectors.

    Provides semantic/keyword/hybrid search over indexed chunks, path listing,
    content reading (cat/head/tail), pattern matching (grep), and full export.
    Reads from Milvus for indexed content and from live connector plugins for
    on-demand reads, with artifact cache support for converted content.
    """
    ...
    async def _has_registered_search_scope(self, connector_uri: str | None) -> bool:
        ...
        if connector_uri is None:
            return await self._obj.has_any_connector()
        return await self._obj.has_connector_uri(connector_uri)
    ...
    async def ls(self, path: str) -> dict:
        ...
        for e in entries:
            ...
            row = await self._obj.get_object_search_status(cid, child_rel)
            out.append(
                {
                    "name": e.name,
                    "type": e.type,
    ...
    async def cat(
        ...
        try:
            ...
            if ext in CONVERT_EXTS:
                ...
                if await self._art.converted_md_stale(cid, rel, st.fingerprint):
                    raw = bytearray()
                    async for ch in plugin.read(rel):
                        raw += ch
                    text = await self._infra.converter.convert(bytes(raw), ext)
                else:
                    art = await self._art.read_artifact(self._ns, curi + rel, "converted_md")
                    if art is not None:
    ...
    async def head(self, path: str, n: int = 20) -> str:
        ...
        try:
            ...
            if structured:
                ...
                art = await self._art.read_artifact(self._ns, curi + rel, "head_cache")
                if art is not None:
    ...
    async def grep(
        ...
        try:
            ...
            not_idx = await self._obj.list_not_indexed_in_scope(cid, rel)
