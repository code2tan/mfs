...
class Engine:
    """Facade：对外保持 18 个公共方法 + ``meta`` 属性，内部委派给协作组件（§4.1）。

    阶段 3：基础设施栈（8 个客户端构建 + connect/close）收口至 ``InfraStack``，进程单例
    pipeline 状态与生命周期收口至 ``PipelineSupervisor``。本类保留普通属性别名（客户端）
    + 三个只读 property（``_pending_finalize`` / ``_embed_consumer`` / ``_job_lane``）+ 薄
    委派，供留在 Engine 的阶段 4+ 方法与 ``tests/`` 直接访问，行为零变化。"""

    def __init__(self, cfg: ServerConfig):
        self.cfg = cfg
        self.ns = cfg.namespace
        # --- 基础设施栈（阶段 3）：8 个客户端构建 + connect/close 生命周期收口于此 ---
        # Engine 保留普通属性别名（非 property），以便测试 ``eng.milvus = _FakeMilvus()`` 等
        # 重赋值后，PipelineSupervisor 经 ``self._engine.<client>`` 仍能观察到 fake。
        self.infra = InfraStack(cfg)
        self.meta = self.infra.meta
        self.milvus = self.infra.milvus
        self.artifact_cache = self.infra.artifact_cache
        self.tx_cache = self.infra.tx_cache
        self.embed = self.infra.embed
        self.converter = self.infra.converter
        self.vlm = self.infra.vlm
        self.summary = self.infra.summary
        self._artifact_writes = 0  # throttles LRU eviction sweeps
        # --- 协作组件（阶段 0 脚手架 + 阶段 1/2/3 逐步迁入实现）---
        # 进程单例 pipeline 状态与生命周期已收口至 PipelineSupervisor（阶段 3）；Engine 经
        # property / 薄委派访问，行为零变化。构造顺序对齐 §4.1 蓝图。
    ...
    # --- 进程单例字段转发（阶段 3）：物理状态在 PipelineSupervisor，留在 Engine 的阶段 4
    # 方法（_drain_job / _finalize_job / _process_with_retry / _run_job_loop）经此只读访问。
    # _pending_finalize 经 property 返回同一 dict，``[uri]=`` / ``.pop()`` 原地变更，无需 setter。---
    @property
    def _pending_finalize(self) -> dict[str, tuple]:
        return self.pipeline._pending_finalize
    ...
    @_job_lane.setter
    def _job_lane(self, value):
        # 测试注入 fake job_lane（如 _NoopReduce）以跳过真实 pipeline 构建（§7 阶段 3 测试契约）。
        self.pipeline._job_lane = value
    ...
    @property
    def _embed_idle_ms(self):
        return self.pipeline._embed_idle_ms

    @_embed_idle_ms.setter
    def _embed_idle_ms(self, value):
        # 测试在 _build_pipeline 前调小 idle 以快速排空（§7 阶段 3 测试契约）；须写入
        # supervisor 字段，_build_pipeline 才能读到。
        self.pipeline._embed_idle_ms = value
    ...
    async def startup(self, *, preload_local_models: bool = False) -> None:
        ...
        await self.pipeline.startup()  # build + gc_orphan + recover_job_lane + watcher

    async def shutdown(self) -> None:
        await self.pipeline.shutdown()  # watcher + job_lane + embed_consumer
        await self.infra.shutdown()  # meta.close + tx_cache.close

    # --- pipeline 进程单例（阶段 3：实现迁入 PipelineSupervisor，薄委派供 tests/ 直接调用）---
    def _build_pipeline(self) -> None:
        self.pipeline._build_pipeline()

    def _routes_to_pipeline(self, okind: str) -> bool:
        return self.pipeline.routes_to_pipeline(okind)

    async def _gc_orphan_chunks(self) -> int:
        return await self.pipeline._gc_orphan_chunks()

    async def _recover_job_lane(self) -> None:
        await self.pipeline._recover_job_lane()
    ...
    async def _on_pipeline_object_indexed(
        ...
        await self.pipeline._on_object_indexed(
            task_uri, job_id, chunk_count=chunk_count, partial=partial, error=error
        )
    ...
    async def _write_object_row(
        ...
        chunk_count). Shared by the inline _index_object tail, the rename branch, and the
        pipeline success hook. SQL 已迁入 ``ObjectRepository``（阶段 2）。"""
    ...
    async def _open_sync_job(self, cid: str, process: bool) -> str:
        ...
        nothing half-applied. SQL 已迁入 ``ObjectRepository.open_sync_job``（阶段 2）。"""
    ...
    async def _finalize_job(self, job_id: str, aborted: str | None) -> None:
        """Set terminal job status + per-status object counts. SQL 已迁入
        ``ObjectRepository.finalize_job``（阶段 2）；本方法保留 job_lane evict 编排。"""
    ...
    async def cancel_job(self, job_id: str) -> bool:
        ...
        if status is None or status in ("succeeded", "failed", "cancelled"):
            return False
    ...
    async def _claim_queued_job(self) -> dict | None:
        ...
        *this* worker's UPDATE flipped the row (rowcount == 1). Two workers racing the
        ...
        SQL 已迁入 ``ObjectRepository.claim_queued_job``（阶段 2）。"""
    ...
    async def run_worker_once(self) -> str | None:
        ...
        try:
            ...
            await self.objects.fail_running_tasks_for_job(job["id"], reason)
            await self.objects.fail_inflight_job(job["id"], reason)
            print(
    ...
    async def _claim_batch(self, limit: int, connector_id: str) -> list[dict]:
        ...
        SQL 已迁入 ``ObjectRepository.claim_tasks``（阶段 2，合并 ``_claim_rows``）。"""
    ...
    async def _should_stop(self, job_id: str, cid: str) -> bool:
        ...
        jr = await self.objects.get_job_status(job_id)
        if jr == "cancelled":
            return True
        cr = await self.objects.get_connector_status(cid)
        return cr == "removing"
    ...
    async def remove_connector(self, target: str) -> bool:
        ...
        # 3. metadata rows — three target tables via the repo; connector_state / file_state /
        # connectors handled here (not the repo's three-table scope).
        await self.objects.delete_object_task_job_rows_for_connector(cid)
        for tbl in ("connector_state", "file_state"):
            await self.meta.execute(f"DELETE FROM {tbl} WHERE connector_id=?", (cid,))
